...
mod tests {
    ...
    fn test_create_and_list_reminders() {
        ...
        let mut stmt = conn.prepare("SELECT id, title FROM reminders WHERE is_active=1").unwrap();
        let ids: Vec<String> = stmt.query_map([], |row| row.get(0)).unwrap()
    ...
    fn test_toggle_reminder() {
        ...
        let active: i32 = conn.query_row("SELECT is_active FROM reminders WHERE id=?1", params!["r1"], |row| row.get(0)).unwrap();
        assert_eq!(active, 0);
    }
    ...
    fn test_delete_reminder() {
        ...
        conn.execute("DELETE FROM reminders WHERE id=?1", params!["r1"]).unwrap();

        let count: i32 = conn.query_row("SELECT COUNT(*) FROM reminders", [], |row| row.get(0)).unwrap();
        assert_eq!(count, 0);
    }
    ...
    fn test_update_next_trigger() {
        ...
        let conn = db.conn.lock().unwrap();
        let next: String = conn.query_row("SELECT next_trigger_at FROM reminders WHERE id=?1", params!["r1"], |row| row.get(0)).unwrap();
    ...
    fn test_recurring_reminder_fields() {
        ...
        let interval: Option<i64> = conn.query_row(
            "SELECT interval_seconds FROM reminders WHERE id=?1", params!["r1"], |row| row.get(0)
        ).unwrap();
