    ...
    std::thread::spawn(move || {
        loop {
            ...
            for reminder in reminders {
                if let Ok(trigger_time) = chrono::DateTime::parse_from_rfc3339(&reminder.next_trigger_at) {
                    if now >= trigger_time {
                        ...
                        if reminder.reminder_type == "recurring" {
                            ...
                            update_next_trigger(&db, &reminder.id, &(now + chrono::Duration::days(365)).to_rfc3339());
                        }
                    }
                }
            }
